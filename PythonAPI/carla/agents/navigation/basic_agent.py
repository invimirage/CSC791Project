...
class BasicAgent(object):
    ...
    def _affected_by_stop_sign(self, stop_distance, stop_signs_list=None, visited_stop_signs=None):
        """
        ...
        if not visited_stop_signs:
            visited_stop_signs = set()

        vehicle_transform = self._vehicle.get_transform()
        vehicle_location = vehicle_transform.location

        for sign in stop_signs_list:
            if sign.id in visited_stop_signs:
                # Already stopped here, won't stop again
                continue
