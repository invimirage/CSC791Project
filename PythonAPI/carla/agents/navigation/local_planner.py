...
class LocalPlanner(object):
    ...
    def run_step_agressive(self, debug=False):
        """
        ...
        """
        # Adjust target speed if following speed limits
        if self._follow_speed_limits:
            self._target_speed = self._vehicle.get_speed_limit()

        # Ensure that there are enough waypoints in the queue.
        ...
        # Purge obsolete waypoints based on distance to the vehicle.
        veh_location = self._vehicle.get_location()
        vehicle_speed = get_speed(self._vehicle) / 3.6  # Convert m/s to km/h
        self._min_distance = self._base_min_distance + self._distance_ratio * vehicle_speed

        num_waypoints_removed = 0
        for waypoint, _ in self._waypoints_queue:
            if len(self._waypoints_queue) - num_waypoints_removed == 1:
                min_distance = 1  # Do not remove the last waypoint until very close
            else:
                min_distance = self._min_distance

            if veh_location.distance(waypoint.transform.location) < min_distance:
                num_waypoints_removed += 1
            else:
                break

        for _ in range(num_waypoints_removed):
            self._waypoints_queue.popleft()

        # Emergency stop if no waypoints remain.
        if len(self._waypoints_queue) == 0:
            ...
            # Create an aggressive longitudinal controller.
            # Note: Ensure that PIDLongitudinalController is imported from agents.navigation.controller.
            from agents.navigation.controller import PIDLongitudinalController
            aggressive_lon_controller = PIDLongitudinalController(self._vehicle,
                                                                    K_P=1.0, K_I=0.0, K_D=0.0, dt=0.03)

            # Compute the aggressive longitudinal control (acceleration).
            acceleration = aggressive_lon_controller.run_step(self._target_speed, debug=debug)

            # Use the existing lateral controller for steering.
            current_steering = self._vehicle_controller._lat_controller.run_step(self.target_waypoint)
            ...
            # Don't Smooth the steering command.
            # past_steering = self._vehicle_controller.past_steering
            ...
            if current_steering >= 0:
                control.steer = min(self._max_steer, current_steering)
            else:
                control.steer = max(-self._max_steer, current_steering)

            control.hand_brake = False
            control.manual_gear_shift = False

            # Update past steering for smoothing in subsequent steps.
            self._vehicle_controller.past_steering = control.steer
